---
title: "KFunc 'bpf_get_mem_cgroup'"
description: "This page documents the 'bpf_get_mem_cgroup' eBPF kfunc, including its definition, usage, program types that can use it, and examples."
---
# KFunc `bpf_get_mem_cgroup`

<!-- [FEATURE_TAG](bpf_get_mem_cgroup) -->
[:octicons-tag-24: v7.0](https://github.com/torvalds/linux/commit/5904db9891f80f84283648121e2d8c8a506296a8)
<!-- [/FEATURE_TAG] -->

Get a reference to a memory cGroup.

## Definition

It's fine to pass a cGroup subsystem state which belongs to any cGroup controller, e.g. unified hierarchy's main cGroup subsystem state.

Implements [`KF_ACQUIRE`](../concepts/kfuncs.md#kf_acquire) semantics.
...
A pointer to a `struct mem_cgroup` after bumping the corresponding cGroup subsystem state's reference counter.

**Signature**

<!-- [KFUNC_DEF] -->
`#!c struct mem_cgroup *bpf_get_mem_cgroup(struct cgroup_subsys_state *css)`
